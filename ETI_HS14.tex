...
$\sum^{n}_{k=0}{3^k}$\\
\\
Haskell: $sum[3i \:\vert\: i \leftarrow [0..12]]$\\
\\
\\
\uline{Varianten von TM}\\
\\
1. Mehrspurmaschine\\
\\
...
\\
\\
\rule{\textwidth}{0.4mm}\\\textbf{}
\\
\uline{Church-Turing-These}\\
Jede im intuitiven Sinne berechenbare Funktion lässt sich durch eine TM berechnen.\\
\\
$\qed$\\
\\
\uline{Codierungen, Beispiel TM}\\
\\
$\Sigma_u = \{0,1,\textbf{,},R,L,A,J,B,C,Q,D,W\}$\\
\\
$\vert\Sigma_u\vert=12$\\
\\
$B0^s1^{(g-1)-s}\textbf{,}C0^c\textbf{,}\underbrace{Q(A\vert J\vert 0^q \textbf{,})}_{\text{Startzustand}}D \underbrace{trans_0 ... trans_1}_{\text{$\delta$}}\color{red}*$\\
$s = \vert \Sigma \vert$\\
$g = \vert \Gamma \vert$\\
$c = \vert C \vert$\\
$C = Q - \{q_{acc}, q_{rej}\}$\\
$\underbrace{\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab}_{\text{$<M>\:\leftarrow$  Codierung von $M$}}$\\
\\
\color{red}{*}\color{black} $\underbrace{sym_0 \textbf{,} ... \textbf{,} sym_{w-1} \textbf{,} \textvisiblespace}_{\text{$<w>$ Wort}}$\\
\\
$\underbrace{\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab\tab}_{\text{$<M, w>$}}$\\
\\
\\
$trans ::= \underbrace{0^i}_{\text{$q_i \in C$}}\textbf{,}\underbrace{0^j}_{\text{$s \in \Gamma$}}\textbf{,}\underbrace{(A\vert J\vert 0^k \textbf{,})}_{\underbrace{\text{$acc \:\vert\: rej \:\vert\: q_k$}}_{\text{$\in Q$}}}\underbrace{0^l}_{\text{$q_l \in \Gamma$}} \textbf{,} \underbrace{(L\vert R)}_{\text{links / rechts}}$\\
\\
\uline{Beispiel}\\
\\
$00\textbf{,}0\textbf{,}A0\textbf{,}L$\\
\\
codiert\\
\\
$\delta(q_2,s_1) = (q_{acc},s_1,L)$\\
\\
$sym ::= \underbrace{0^i}_{\text{$s_1 \in \Sigma$}}$\\
\\
\\
\rule{\textwidth}{0.4mm}\\
\\
\uline{Universalmaschine $U$}
