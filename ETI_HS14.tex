...
\uline{Beweis:}\\
\\
Sei M ein DFA das L erkennt, $M = \{Q,\sum,\delta,q_0,F\}$\\
\\
Sei p die Anzahl Zustände : $p = |Q|$\\
\\
Sei $ s = s_1 \hdots s_n \in L$ mit $ s_i \in \sum $ und $ n \ge p$\\
\\
...
Länge $n+1 \ge p+1, n+1 > p$\\

Da $M$ $p$ Zustände hat muss \textbf{mindestens ein Zustand zweimal unter dem erstem p+1 Zuständen vorkommen /durchgelaufen werden} (Eigenschaften des DFA - jedes Zeichen des Alphabet muss durch jeder Zustand verarbeitet werden)\\

\textbf{Pidgeonhole Prinzip:}\\
Beziechne $r_j$ das erste Auftreten eines sollchen Zustandes (Zustände paarweise verschieden bis zum gewissen Punkt). $r_l$ das zweite Auftreten des Zuständes $l>j$\\
\newpage
(Diagram hier)\\
